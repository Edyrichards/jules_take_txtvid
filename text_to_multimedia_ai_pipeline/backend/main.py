...
class VideoRequest(BaseModel):
    image_path: str           # Path to the input image (from Phase 2)
    motion_type: str          # Currently conceptual, could map to AD parameters later
    prompt: Optional[str] = "A beautiful animation" # Add prompt for AD

# New imports for MLX AnimateDiff
from . import config_i2v
from . import mlx_image_to_video
...
async def generate_video(request: VideoRequest):
    print(f"Received video generation request for image: '{request.image_path}', motion: '{request.motion_type}', prompt: '{request.prompt}'")

    actual_image_path_server = os.path.join(PROJECT_ROOT_DIR, request.image_path)
    if not os.path.exists(actual_image_path_server):
        raise HTTPException(status_code=404, detail=f"Input image not found at: {request.image_path}")

    try:
        input_pil_image = Image.open(actual_image_path_server)

        start_time = time.time()

        # Call the MLX AnimateDiff frame generation function
        generated_frames = mlx_image_to_video.generate_video_frames_with_mlx(
            input_pil_image=input_pil_image,
            prompt=request.prompt if request.prompt else "animated video",
            num_frames=config_i2v.NUM_FRAMES
        )

        frames_gen_time = time.time() - start_time
        print(f"MLX AnimateDiff frames generated in {frames_gen_time:.2f} seconds (placeholder timing).")

        if not generated_frames:
            raise HTTPException(status_code=500, detail="Video frame generation failed (returned no frames).")

        # Compile frames into a video file
        timestamp = time.strftime("%Y%m%d-%H%M%S")
        sane_prompt_part = "".join(c if c.isalnum() else "_" for c in (request.prompt or "video")[:20]).rstrip("_")
        video_filename = f"i2v_{sane_prompt_part}_{timestamp}.{config_i2v.OUTPUT_VIDEO_FORMAT.lower()}"

        output_video_path_server = os.path.join(GENERATED_VIDEOS_DIR_SERVER, video_filename)
        output_video_path_client = os.path.join("data/generated_videos", video_filename)

        compile_start_time = time.time()
        mlx_image_to_video.compile_frames_to_video(
            generated_frames,
            output_video_path_server,
            config_i2v.FPS
        )
        compile_time = time.time() - compile_start_time
        print(f"Video compiled in {compile_time:.2f} seconds.")
        ...
        return {
            "message": f"Video generated successfully with MLX AnimateDiff in {total_time:.2f}s (Placeholder Models).",
            "video_path": output_video_path_client,
            "num_frames": len(generated_frames),
            "fps": config_i2v.FPS,
            "base_resolution": f"{generated_frames[0].width}x{generated_frames[0].height}",
            "frame_interpolation_status": frame_interpolation_status,
            "video_upscaling_status": video_upscaling_status,
            "prompt_used": request.prompt,
            "motion_type_logged": request.motion_type,
            "generation_time_seconds": round(total_time, 2)
        }

    except FileNotFoundError as fnf_error:
        print(f"Model FileNotFoundError in /generate-video: {fnf_error}")
        raise HTTPException(status_code=500, detail=f"Model files not found for AnimateDiff or its base SD model. Please check server logs. Error: {fnf_error}")
    except IOError as io_error:
        print(f"IOError in /generate-video (likely OpenCV VideoWriter): {io_error}")
        raise HTTPException(status_code=500, detail=f"Failed to write video file: {str(io_error)}")
    except RuntimeError as e:
        print(f"RuntimeError in /generate-video: {e}")
        import traceback
        traceback.print_exc()
        error_detail = str(e)
        if "MPS" in error_detail or "mlx" in error_detail or "metal" in error_detail.lower(): # Added metal
            error_detail = "An error occurred with the MLX/MPS backend during video generation. Check server logs."
        raise HTTPException(status_code=500, detail=error_detail)
    except Exception as e:
        print(f"General Exception in /generate-video: {e}")
        import traceback
        traceback.print_exc()
        raise HTTPException(status_code=500, detail=f"An unexpected error occurred during video generation: {str(e)}")
